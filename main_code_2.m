...
close all;
clear all;
%% If Noise used just in the beginging then everything works well
...
mod_data=qammod(data,M);ep_new=zeros(1,30);ep=ones(1,30);ep=0.30*(ep);error_offset=zeros(1,30);
%% Noise
    for snr_db=1:30
    sig_power=bandpower(mod_data);
    snr_l=10.^(snr_db/10);
    var=sig_power/snr_l;
    noise=(1/sqrt(2)*(randn(1,N-No_pilot)+j*randn(1,N-No_pilot)))*var;
    noisy_sig=mod_data+noise;
%% Providing frequency offset
 ep(snr_db)=0.15;
 mm=(0:(N-No_pilot)-1);
 offset = exp(i*2*pi*ep(snr_db)/(N-No_pilot).*mm);
 fil_data=noisy_sig.*offset;
...
%% Calculate offset
     mm=(0:(N-No_pilot)-1);
     ep_new(snr_db)=new_extended_kalmaan(Data_extracted,(mod_data),var);
      offset_cal = exp(-i*2*pi*ep_new(snr_db)/(N-No_pilot).*mm);
     de_offset_data=Data_extracted.*offset_cal;
     error_offset(snr_db)=((ep(snr_db)-ep_new(snr_db))/ep(snr_db))*100;
% %% Equalization
% lin_eq = lineareq(8, lms(0.01)); % Create an equalizer object.
% eql_data= equalize(lin_eq,de_offset_data,mod_data(1:25)); % Equalize.
